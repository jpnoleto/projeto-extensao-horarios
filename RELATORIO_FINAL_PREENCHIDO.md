...
> Atividade desenvolvida em modalidade híbrida: o sistema foi desenvolvido em ambiente próprio do aluno (Polo `<seu polo>`) e disponibilizado, via hospedagem web gratuita (Render.com como servidor de aplicação e freedb.tech como banco de dados MySQL — `<URL final do app>`), para uso pela `<nome da escola pública parceira>`, localizada em `<cidade/UF>`. O código-fonte ficou publicado em <https://github.com/Noletinho/projeto-extensao-horarios>, ampliando o alcance da ação para qualquer escola interessada.
...
>
> 7. **Hospedagem em ambiente público gratuito** (Render.com para a aplicação Flask e freedb.tech para o banco MySQL) e publicação do código-fonte sob licença aberta no GitHub, garantindo que qualquer escola interessada possa adotar a ferramenta sem custos.
>
> 8. **Apresentação à equipe gestora** da escola parceira, com treinamento básico de uso e coleta de feedback.

### Caso necessário, houve mudança de estratégia para alcançar o resultado

> Inicialmente a hospedagem do sistema seria realizada na plataforma Railway, escolhida pela documentação técnica do projeto. Durante o desenvolvimento, contudo, a plataforma encerrou seu plano gratuito permanente, oferecendo apenas crédito limitado de avaliação. Avaliou-se em seguida o PythonAnywhere, que passou a restringir o serviço de banco de dados a planos pagos no decorrer do projeto. Para preservar o caráter gratuito da entrega — premissa fundamental do projeto, voltado a escolas públicas sem orçamento —, optou-se pela arquitetura final de hospedagem distribuída entre dois provedores gratuitos: a aplicação Flask publicada no **Render.com** (Web Service Free, sem custo permanente) e o banco de dados MySQL fornecido pelo **freedb.tech** (200 MB gratuitos, suficientes para a base cadastral de uma escola). A mudança exigiu adaptação das variáveis de ambiente e revisão da documentação de deploy.

### Resultado da ação

> Foi entregue à escola parceira um sistema web completo, hospedado online e disponibilizado gratuitamente, capaz de:
>
...
>
> Sob a perspectiva socioemocional, o contato com a equipe pedagógica da escola parceira evidenciou a importância da comunicação interpessoal. Foi necessário traduzir requisitos pedagógicos vagos em especificações técnicas precisas, ouvir as preocupações dos professores quanto ao novo fluxo e ajustar repetidamente a interface para que se tornasse intuitiva para pessoas sem perfil técnico. A flexibilidade e a capacidade de adaptação foram mobilizadas, sobretudo, quando duas das plataformas de hospedagem inicialmente avaliadas (Railway e PythonAnywhere) deixaram de oferecer seus recursos gratuitos durante o desenvolvimento, exigindo um redesenho rápido da arquitetura de deploy — que passou a combinar o Render.com para a aplicação Flask e o freedb.tech para o banco MySQL — sem comprometer o cronograma da entrega.
>
